...
if __name__ == "__main__":
    ...
    # test_df = read_csv("../Data/Task{}_test-without-labels.csv".format(args.task), encoding="utf8")
    # test_dataset = ClassificationDataset(f_subtask=args.task,
    #                                      f_tokenizer=tokenizer,
    #                                      f_data=test_df,
    #                                      f_max_length=args.max_length,
    #                                      f_y=None)
    test_df = read_csv("../data/Task{}_dev.csv".format(args.task), encoding="utf8")
    ...
    for aspect in ["Validity", "Novelty"]:
        ...
        train_df = read_csv("../data/Task{}_train.csv".format(args.task), encoding="utf8")
        if args.task == "A":
        ...
        train_dataset = ClassificationDataset(f_subtask=args.task,
                                              ...
                                              f_max_length=args.max_length,
                                              f_y=aspect)
        dev_df = read_csv("../data/Task{}_dev.csv".format(args.task), encoding="utf8")
        ...
        all_predictions = []
        prediction_mapper = {0: -1, 1: 1}
        # test_dataset is now dev set
        for i in range(0, len(test_dataset), 16):
            input_ids = torch.stack([test_dataset[j]["input_ids"] for j in range(i, min(i+16, len(test_dataset)))], dim=0)
